import sentitiveWord from './sentitiveWord'

function makeSensitiveMap() {
  const sensitiveWordList = sentitiveWord
  // 构造根节点
 ...
 * @returns
 */
function checkSensitiveWord(sensitiveMap: any, txt: any, index: any) {
 ...
 * @returns
 */
export function filterSensitiveWord(txt: any, sensitiveMap?: any) {
  sensitiveMap = makeSensitiveMap()
 ...
 */
export function myReplace(sInput: any, sChar: any, sReplaceChar: any) {
  if (sInput == '' || sInput == undefined) {
    return ''
  }

  const oReg = new RegExp(sChar, 'g')
